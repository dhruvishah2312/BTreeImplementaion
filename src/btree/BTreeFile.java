...
public class BTreeFile extends IndexFile implements GlobalConst {
...

	{
		...
		empty_page = pinPage(currentPageId);
		BTSortedPage currentpage = new BTSortedPage(empty_page, AttrType.attrInteger);
		KeyDataEntry upEntry = null;
		if(currentpage.getType() == NodeType.INDEX)
		{
			BTIndexPage currentindexPage = new BTIndexPage(empty_page, AttrType.attrInteger);
			...
			else
			{
				...
				{
					BTIndexPage newindexPage = new BTIndexPage(empty_page, AttrType.attrInteger); 
					PageId newindexPageid = newindexPage.getCurPage();
					RID delRID = new RID();
					KeyDataEntry tempkeyDataEntry = currentindexPage.getFirst(delRID);
					...
					count = (newindexPage.getSlotCnt() + 1) / 2 ;
					
					for(tempkeyDataEntry = newindexPage.getFirst(delRID); splitPointer <= count-1; splitPointer++) {
					currentindexPage.insertKey(key, CurrentIndexpageid);
					newindexPage.deleteSortedRecord(delRID);
				}
					if(BT.keyCompare(upEntry.key, tempkeyDataEntry.key) > 0) {
						newindexPage.insertKey(upEntry.key, currentPageId);
					} else {
						currentindexPage.insertKey(upEntry.key, currentPageId);
					}
					unpinPage(CurrentIndexpageid, true);
					upEntry = newindexPage.getFirst(delRID);
					newindexPage.setPrevPage(CurrentIndexpageid);
					newindexPage.deleteSortedRecord(delRID);
			...
			BTLeafPage currentLeafPage = new BTLeafPage(empty_page, AttrType.attrInteger);
			...
			if(currentLeafPage.available_space() >= BT.getKeyDataLength(upEntry.key, NodeType.LEAF)) {
				...
				BTLeafPage newLeafPage = new BTLeafPage(empty_page, AttrType.attrInteger); 
				PageId newLeafPageId = newLeafPage.getCurPage();
				newLeafPage.setNextPage();
				newLeafPage.setPrevPage();
				currentLeafPage.setNextPage(newLeafPageId);
				RID delRID = new RID();
				KeyDataEntry tempkeyDataEntry = currentLeafPage.getFirst(delRID);
				...
				for (tempkeyDataEntry = currentLeafPage.getFirst(delRID);	tempkeyDataEntry != null; tempkeyDataEntry = currentLeafPage.getFirst( delRID)){
					newLeafPage.insertRecord(tempkeyDataEntry);
					currentLeafPage.deleteSortedRecord(delRID);
				}
				
				count = (newLeafPage.getSlotCnt() + 1) / 2;
				
				for(tempkeyDataEntry = newLeafPage.getFirst(delRID); splitPointer <= count-1; splitPointer++) {
					currentLeafPage.insertRecord(tempkeyDataEntry);
	...
	// remove the return statement and start your code.
			//return false;
		try {
			BTLeafPage ResultLeafPage = new BTLeafPage(AttrType.attrInteger);
			ResultLeafPage = findRunStart(key, rid);
			if (ResultLeafPage == null) {
				System.out.println("Record doesn't exist");
				return false;
			}
			else {
				if(BT.keyCompare(key, ResultLeafPage.getCurrent(rid).key) == 0) {
					pinPage(ResultLeafPage.getCurPage());
					ResultLeafPage.deleteSortedRecord(rid);
					unpinPage(ResultLeafPage.getCurPage(), true);
